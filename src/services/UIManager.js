(function () {
    const Utils = window.KapturUtils;
    ...
    const LARGE_PREVIEW_ID = "kaptur-recorder-large-preview";
    ...
    function getTrustedHTML(htmlString) {
        if (window.trustedTypes && window.trustedTypes.createPolicy) {
            if (!trustedTypesPolicy) {
                try {
                    trustedTypesPolicy = window.trustedTypes.createPolicy('kapturRecorderPolicy', {
    ...
    class KapturUI {
        ...
        static getInstance() {
            if (!window.KapturUIInstance) window.KapturUIInstance = new KapturUI();
            return window.KapturUIInstance;
        }
        ...
        async init() {
            ...
            this.container.id = "kaptur-recorder-wrapper-v3";
        ...
        _injectGlobalStyles() {
            if (document.getElementById("kaptur-styles")) return;
            const style = document.createElement("style");
            style.id = "kaptur-styles";
            ...
            style.textContent = `
                ...
                .kaptur-rounded-btn:active { transform: scale(0.95); }
                
                .pause svg, .play svg { fill: #555; }
                .submit svg { fill: #00C0A3; }
                ...
                #${C.UI.WEBCAM_PREVIEW_ID}:hover .kaptur-resize-handle,
                #${LARGE_PREVIEW_ID}:hover .kaptur-resize-handle {
                    opacity: 1;
                }

                #${C.UI.COUNTDOWN_ID} {
                    position: fixed; inset: 0; margin: auto; width: 200px; height: 200px;
                    background: #00C0A3; border-radius: 50%; display: grid; place-items: center;
                    color: white; font-size: 100px; font-weight: bold; font-family: sans-serif;
                    box-shadow: 0 10px 30px rgba(0, 192, 163, 0.4); opacity: 0; transition: opacity 0.4s; z-index: 2147483647;
                }
            `;
        ...
        async showControls(onActionCallback) {
            ...
            const htmlContent = `
                ...
                <div class="elapsed-time">
                    ...
                    <button class="kaptur-rounded-btn pause" id="btn-pause" title="Pausar">${ICONS.PAUSE}</button>
                    <button class="kaptur-rounded-btn play" id="btn-resume" title="Retomar" style="display: none;">${ICONS.PLAY}</button>
                </div>
                <div class="space"></div>
                <div class="actions">
                    <button class="kaptur-rounded-btn submit" id="btn-stop" title="Finalizar">${ICONS.STOP}</button>
                    <button class="kaptur-rounded-btn delete" id="btn-delete" title="Cancelar">${ICONS.TRASH}</button>
                </div>
            `;
        ...
        async _createResizableVideo(stream, elementId) {
            ...
            video.className = "kaptur-video-element";

            const handle = document.createElement("div");
            handle.className = "kaptur-resize-handle";
        ...
        async cleanup() {
            ...
            const st = document.getElementById("kaptur-styles");
            if(st) st.remove();
        }
        ...
        _makeDraggable(element) {
            ...
            const onMouseDown = (e) => {
                if (e.target.closest('.kaptur-resize-handle')) return;
    ...
    window.KapturUIManager = KapturUI;
})();